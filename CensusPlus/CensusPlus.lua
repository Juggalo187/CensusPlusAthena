...
--
---------------------------------------------------------------------------------
local CensusPlus_VERSION = "1.0"; 			-- version
...
local skulyprefix = "skulycensus";
local playerName = UnitName("player");
local CfactionName = UnitFactionGroup("player");
...
function CensusPlus_GetALLRaces(faction)
	local ret = {CENSUSPlus_ORC, CENSUSPlus_TAUREN, CENSUSPlus_TROLL, CENSUSPlus_UNDEAD, CENSUSPlus_BLOODELF, CENSUSPlus_GOBLIN, CENSUSPlus_DWARF, CENSUSPlus_GNOME, CENSUSPlus_HUMAN, CENSUSPlus_NIGHTELF, CENSUSPlus_DRAENEI, CENSUSPlus_WORGEN};
...
--		CensusPlus_DetermineServerDate();
	...
	elseif( command ~= nil and string.lower(command) == "timer" ) then
		if( value ~= nil ) then
			...
			CensusPlus_Database["Info"]["AutoCensusTimer"] = 300;
			CensusPlus_Msg( "Set autocensus timer to 5 minutes" );
		end
...
function CensusPlus_StopCensus(  )
	if (g_IsCensusPlusInProgress) then
		...
		CensusPlusScanProgress:SetText("No Scan In Progress");
...
function CensusPlus_InitializeVariables()
	...
	if( CensusPlus_Database["Info"]["SkulysaveTotal"] == nil ) then
		CensusPlus_Database["Info"]["SkulysaveTotal"] = 0;
	end
...
function CensusPlus_OnUpdate()
	...
	if (CensusPlus_Database["Info"]["AutoCensus"] == true and not g_IsCensusPlusInProgress) then

			
			local timerdisplayed = CensusPlus_Database["Info"]["AutoCensusTimer"] / 60;
			local skulytest1 = (time() - (g_LastCensusRun));
			
			
			
			local minutes = (SecondsToClock(skulytest1));
			
			CensusPlusTimer:SetText(format(CENSUSPlus_TIMER, timerdisplayed, minutes));
			else
			CensusPlusTimer:SetText("");
			
	end
	
	SkulysaveTotals = CensusPlus_Database["Info"]["SkulysaveTotal"];
...
local function TotalsAccumulator(name, level, guild, faction)
	--
	--  Add character to our player list
	--
	CensusPlus_AddPlayerToList( name, level, guild, faction );
...
function CensusPlus_UpdateView()
	...
	CensusPlusRealmName:SetText("|CFF87CEEB"..CENSUSPlus_REALMNAME.."|r|CFFDEB887"..realmName.."|r");
	local factionGroup = UnitFactionGroup("player");
	if( factionGroup == nil ) then
		return;
	end
	...
	local Skuly_faction_color
	
	if factionGroup == "Horde" then
		Skuly_faction_color = "|CFFFF0000"
	 end
	if factionGroup == "Alliance" then
		Skuly_faction_color = "|CFF1E90FF"
	end
	
	CensusPlusFactionName:SetText("|CFF87CEEB"..CENSUSPlus_FACTION.."|r"..Skuly_faction_color..factionGroup.."|r");
	
	
	if( CensusPlus_Database["Info"]["Locale"] ~= nil ) then
		--CensusPlusLocaleName:SetText(format(CENSUSPlus_LOCALE, CensusPlus_Database["Info"]["Locale"]));
		CensusPlusLocaleName:SetText("|CFF87CEEB"..CENSUSPlus_LOCALE.."|r|CFFDEB887"..CensusPlus_Database["Info"]["Locale"].."");
	end
	...
	local hordeCount = g_RaceCount[1]+g_RaceCount[2]+g_RaceCount[3]+g_RaceCount[4]+g_RaceCount[5]+g_RaceCount[6];
	local alliancecount = g_RaceCount[7]+g_RaceCount[8]+g_RaceCount[9]+g_RaceCount[10]+g_RaceCount[11]+g_RaceCount[12];
	
	local factionGroup = UnitFactionGroup("player");
	
	if hordeCount == 0 and alliancecount > 0 then
	CensusPlusTotalCharacters:SetText("|CFF87CEEB"..totalCharactersText.."|r |CFF1E90FF[Alliance]|r"..alliancecount);
	end
	
	
	if alliancecount == 0 and hordeCount > 0 then
	CensusPlusTotalCharacters:SetText("|CFF87CEEB"..totalCharactersText.."|r |cffFF0000[Horde]|r"..hordeCount);
	end
	
	
	if alliancecount > 0 and hordeCount > 0 then
	CensusPlusTotalCharacters:SetText("|CFF87CEEB"..totalCharactersText.."|r |cffFF0000[Horde]|r"..hordeCount.." |CFF1E90FF[Alliance]|r"..alliancecount);
	end
	--
	-- Update race bars
	--
	...
	--juggalo
	SkulysaveTotals = g_TotalCount
	CensusPlus_Database["Info"]["SkulysaveTotal"] = SkulysaveTotals
	debugprofilestop();
...
function CensusPlus_OnClickLogo( this )
g_RaceSelected = 0;
g_ClassSelected = 0;
g_LevelSelected = 0;
g_GuildSelected = nil;
	...
	while (i <= CensusPlus_NUMGUILDBUTTONS) do
		...
		if (iGuild <= size) then
			...
			if (guild.m_Name == "") then
				getglobal(textField):SetText("|CFFADFF2F"..CENSUSPlus_UNGUILDED.."|r");
			else
				getglobal(textField):SetText("|CFF8FBC8B"..guild.m_Name.."|r");
			end
			--
...
local Skuly_faction_color
	...
	if CfactionName == "Horde" then
		Skuly_faction_color = "|CFFFF0000"
	 end
	if CfactionName == "Alliance" then
		Skuly_faction_color = "|CFF1E90FF"
	end



	if event == "CHAT_MSG_ADDON" and arg1 == skulyprefix and not string.find(arg2, "Reloaded") then
		-- if UnitName("player") == devName or string.find(playerName, "Skuly") then
		-- print(arg2)
		-- end
		for i = 1, getN do
	...
	elseif event == "PLAYER_ENTERING_WORLD" then
			  if (GetNumTalentTabs() >=1) then
				 test = "Census+ User: ["..playerName.."] Reloaded"
			  else
				  test = "Census+ User: ["..Skuly_faction_color..playerName.."- lvl "..Clevel.."|r] Logged In. ".."DBTotal = |CFF1ABD33"..CensusPlus_Database["Info"]["SkulysaveTotal"].."|r"
				 
			  end
			
			RegisterAddonMessagePrefix(skulyprefix)
			for i = 1, getN do
				for k,v in ipairs(T[i]) do
					SendAddonMessage(skulyprefix, test, "WHISPER", v)
				end
			end
...
if GetCVar("realmName") == "Athena" then
